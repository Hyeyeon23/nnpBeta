...
export function PACK250_CF({ imageSrc, color1, ...props }) {
  const { scene, nodes, materials } = useCustomGLTF("/containers/PACK250_CF.glb");
  ...
  scene.traverse((child) => {
    ...
    if (child.isMesh && child.material && child.material.map && child.material.map.name === "프로틴밀250") {
  ...
  useEffect(() => {
    ...
    if (color1) {
      ...
      // 뚜껑주변
      if (materials["매테리얼.004"]) {
        materials["매테리얼.004"].color = new Color(color1);
        materials["매테리얼.004"].needsUpdate = true;
      }
    }
